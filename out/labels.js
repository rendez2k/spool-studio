'use strict';
{
 ...
 <p class="label-hint">Print a small label for each roll, or matching copies for its spool and box. Shelf numbers reflect the current filters and order; they are not permanent spool IDs.</p>
 <div class="label-workspace"><form id="label-settings"><div class="label-fields">
 <label>Label size<select id="label-size"><option value="60x30">60 × 30 mm · spool &amp; box</option><option value="50x30">50 × 30 mm</option><option value="40x30">40 × 30 mm</option><option value="100x50">100 × 50 mm</option><option value="4x6">4 × 6 in · shipping</option><option value="custom">Custom size</option></select></label>
 ...
 function label(row){
  ...
  add('div','label-position','#'+row.position+' · Shelf '+row.shelf+' / '+row.shelfSlot);
  add('strong','label-colour',row.colour);
  add('div','label-brand',row.brand);
  add('div','label-material',[row.product,!row.product.toLowerCase().includes(row.material.toLowerCase())?row.material:'',row.finish!=='unknown'&&!row.product.toLowerCase().includes(row.finish.toLowerCase())?row.finish:''].filter(Boolean).join(' · '));
 ...
 function render(){
  ...
  try{
   ...
   const options=settings();planned=SpoolLabels.plan(snapshot,options);sizeElements(options.width,options.height);
   ...
   node('label-status').textContent=planned.length+' labels · '+(options.end-options.start+1)+' rolls · '+options.width+' × '+options.height+' mm.'+(snapshot.uncounted?' '+snapshot.uncounted+' uncounted bundle'+(snapshot.uncounted===1?'':'s')+' excluded.':'');
