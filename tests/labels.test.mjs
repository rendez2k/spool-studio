...
test('label interactions print paired copies, clear print staging, block stale stock and reject overflow',async()=>{
 ...
 preview=get('label-preview').children[0];
 assert.equal(preview.children.find(child=>child.className==='label-shelf').textContent,'<b>Dry box A</b>');
 assert(preview.children.every(child=>!['label-brand','label-stock'].includes(child.className)));
 assert.equal(get('label-print').disabled,false);
 context.SpoolReels=createRequire(import.meta.url)('../out/reels-core.js');
 context.window.location={origin:'https://spool-studio.uk'};
 vm.runInNewContext(readFileSync(new URL('../out/vendor/qrcode.js',import.meta.url),'utf8'),context);
 selected={...selected,slots:[{...selected.slots[0],reelId:'11111111-1111-4111-8111-111111111111',reelNumber:7,reelLocation:'Drawer 2'}]};
 get('label-location').value='saved';get('label-qr').checked=true;context.window.openSelectedLabels();
 preview=get('label-preview').children[0];
 assert.equal(preview.children[0].textContent,'SP-00007');
 assert.equal(preview.children.find(child=>child.className==='label-shelf').textContent,'Drawer 2');
 assert.match(preview.children.find(child=>child.className==='label-code').innerHTML,/crispEdges/);
 get('label-qr-style').value='rounded';get('label-settings').input();
 assert.match(get('label-preview').children[0].children.find(child=>child.className==='label-code').innerHTML,/data-style="rounded"/);
 get('label-settings').onsubmit({preventDefault(){}});assert.equal(printCount,5);
 assert.equal(body.children[0].children[0].children.find(child=>child.className==='label-code').innerHTML,body.children[0].children[1].children.find(child=>child.className==='label-code').innerHTML);
 events.afterprint();
